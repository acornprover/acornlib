...
- [ ] Relate `is_squarefree` to the prime valuation being at most one. The prime *divisor* form is
      done and needs no factorisation machinery at all:
      `src/nat_squarefree_prime.ac` proves squarefreeness equivalent to no prime square dividing.
      A square divisor above one has a prime factor, and the square of that prime divides the
      square divisor, so the prime condition is no weaker than the general one. Only
      `has_prime_divisor` is used, and it also supplies a prime out of thin air for the `n = 0`
      case, where every square divides.

      The *valuation* form remains blocked, and the cap is real. Adding the six valuation theorems
      to `src/number_theory/interface.ac` fails with "module has more than 500 exported
      declarations". This corrects an earlier note of mine that said the cap was not binding: five
      exports fit in an earlier pass because there was headroom for about that many, not because
      there was no limit, and six more overflow it.

      There is no way around the interface. `from number_theory.factorisation import ...` from
      outside the package fails with "module is private to its package", so the interface is the
      only boundary. Unblocking this means either removing declarations from that interface — a
      decision about what it is for — or raising the compiler's limit.
