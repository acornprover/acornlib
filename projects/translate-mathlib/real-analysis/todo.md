...
- `src/real/continuity_quadratic.ac` defines `quadratic_real(a, b, c)(x) = a * x * x + b * x + c`, identifies it with `add_fns(const_mul_left(a, square_real), affine_real(b, c))`, and proves it is continuous at each point and globally.
- `src/real/continuity_shift.ac` defines `shift_fn(f, h)(x) = f(x + h)`, identifies it with `compose(f, affine_real(1, h))`, and proves that shifting a continuous real function by a real constant preserves continuity at a point and globally.
