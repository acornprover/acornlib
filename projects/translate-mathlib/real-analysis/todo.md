...
- `src/real/continuity_quintic.ac` defines `quintic_real(x) = x * x * x * x * x`, identifies it with `pointwise_mul(quartic_real, identity_fn)`, and proves it is continuous at each point and globally.
- `src/real/continuity_sextic.ac` defines `sextic_real(x) = x * x * x * x * x * x`, identifies it with `pointwise_mul(quintic_real, identity_fn)`, and proves it is continuous at each point and globally.
