...
- [ ] Add further restricted-action support and product-action refinements (orbit-equality transport, transitivity/faithfulness reflection beyond the injectivity bridge)
...
Progress: `src/product_action.ac` now provides the componentwise product action on `Pair[X, Y]`, the bundled `product_action(a, b): MulAction[G, Pair[X, Y]]`, coordinate evaluation lemmas, equivariance of both projections, and basic orbit projection/same-witness introduction lemmas.
