...
- `src/pair_order.ac` now has `pair_meet_new` and `pair_join_new` showing that componentwise meet/join of `Pair.new(a, b)` and `Pair.new(c, d)` equals `Pair.new(a.meet(c), b.meet(d))` and `Pair.new(a.join(c), b.join(d))` respectively, plus direct first/second component projection lemmas `pair_meet_new_first`, `pair_meet_new_second`, `pair_join_new_first`, and `pair_join_new_second`.
- `src/pair_order.ac` now has strict iff characterizations of componentwise pair meet/join versus their arguments: `pair_meet_lt_left_iff_not_lte`, `pair_meet_lt_right_iff_not_lte`, `pair_join_lt_left_iff_not_lte`, and `pair_join_lt_right_iff_not_lte`, plus the matching direct strict-direction wrappers.
- `src/pair_order.ac` now has lattice-level meet-versus-join lemmas for componentwise pair lattices: `pair_lte_meet_join` (meet is always below join), `pair_meet_eq_join_iff_eq` (meet equals join iff the pairs are equal), and `pair_meet_lt_join_iff_ne` (meet is strictly below join iff the pairs differ).
- `src/pair_order.ac` now has direct first/second component projection lemmas for the componentwise meet/join of two diagonal pairs: `pair_meet_diag_first`, `pair_meet_diag_second`, `pair_join_diag_first`, and `pair_join_diag_second`.
- `src/pair_order.ac` now has direct first/second component projection lemmas for mixed diagonal-pair meet/join: `pair_meet_diag_left_first`/`_second`, `pair_meet_diag_right_first`/`_second`, `pair_join_diag_left_first`/`_second`, and `pair_join_diag_right_first`/`_second`.
