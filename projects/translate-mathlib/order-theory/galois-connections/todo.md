...
- `src/galois_connection.ac` now has identity and composition Galois connections: `identity_galois_connection` proves the identity pair is a Galois connection, and `compose_galois_connection` proves that composing two Galois connections gives a Galois connection on the outer types.
- `src/galois_connection.ac` now also has `identity_galois_insertion` and `identity_galois_coinsertion`, showing that the identity function pair is both a Galois insertion and a Galois coinsertion on any partial order.
- `src/galois_connection.ac` now has `compose_galois_insertion` and `compose_galois_coinsertion`, showing that the composition of two Galois insertions (resp. coinsertions) is a Galois insertion (resp. coinsertion).
