...
- `src/fixed_point.ac` now has `common_pre_fixed_point_set` and `common_post_fixed_point_set` mirroring the common fixed-point set API: membership characterizations, commutativity, left/right containment in each map's pre-/post-fixed set, and self-pairing collapse to the single pre-/post-fixed-point set.
