...
- `src/fixed_point.ac` now has direct transitive subset lemmas of the common fixed-point set into each map's pre- and post-fixed-point sets: `common_fixed_point_set_subset_pre_fixed_left`/`_right` and `common_fixed_point_set_subset_post_fixed_left`/`_right`.
- `src/fixed_point.ac` now has `identity_is_pre_fixed_point` and `identity_is_post_fixed_point` mirroring `identity_is_fixed_point` over any `PartialOrder`.
...
- `src/fixed_point.ac` now has `common_pre_fixed_point_set` and `common_post_fixed_point_set` mirroring the common fixed-point set API: membership characterizations, commutativity, left/right containment in each map's pre-/post-fixed set, and self-pairing collapse to the single pre-/post-fixed-point set. It also relates them to the common fixed-point set: the common fixed-point set is contained in both the common pre- and common post-fixed-point sets and equals their intersection, plus identity-pairing specializations for both common pre- and post-fixed-point sets.
- `src/fixed_point.ac` now also has `identity_is_fixed_point` (every point is fixed by the identity), `common_fixed_point_set` specializations (`_self`, `_identity_left`, `_identity_right` reducing to a single fixed-point set), and monotone-map composition lemmas `pre_fixed_point_of_compose` / `post_fixed_point_of_compose` (a common pre-/post-fixed point of two maps with monotone outer map is a pre-/post-fixed point of the composition). (Any goal whose conclusion contains a `compose(...)` lambda as the self-map argument of `is_fixed_point` keeps timing out, so the composition-targeted lemmas are stated only at the pointwise pre-/post-fixed level.)
