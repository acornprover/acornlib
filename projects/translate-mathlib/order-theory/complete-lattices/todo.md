...
- `src/order_bounds.ac` now has matching iff-form pair glb/lub characterizations against the lte hypothesis: `is_glb_is_in_pair_left_iff_lte`, `is_lub_is_in_pair_right_iff_lte`, `is_glb_is_in_pair_right_iff_lte`, and `is_lub_is_in_pair_left_iff_lte`.
- `src/order_bounds.ac` now has direct endpoint comparison projection lemmas for pair glb/lub: `is_glb_is_in_pair_le_left`, `is_glb_is_in_pair_le_right`, `is_lub_is_in_pair_ge_left`, and `is_lub_is_in_pair_ge_right`.
- `src/order_bounds.ac` now has self-pair least/greatest/glb/lub lemmas: `is_least_is_in_pair_self`, `is_greatest_is_in_pair_self`, `is_glb_is_in_pair_self`, and `is_lub_is_in_pair_self` collapse the diagonal pair `is_in_pair(a, a)` to its sole element `a`.
- `src/order_bounds.ac` now has iff characterizations of lower/upper bounds of the diagonal pair: `is_lower_bound_is_in_pair_self_iff_le` and `is_upper_bound_is_in_pair_self_iff_ge` reduce bounds of `is_in_pair(a, a)` to comparison against `a`.
