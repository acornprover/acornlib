...
- [ ] Add `conj` as a ring/field automorphism: function-level `complex_conj_fn` with pointwise add/mul/one/involution lemmas now in `src/complex_conj_hom.ac`; wrapping with `is_ring_hom` at concrete `Complex, Complex` runs into the same predicate-unfolding issue as the `AddGroupHom` packaging
