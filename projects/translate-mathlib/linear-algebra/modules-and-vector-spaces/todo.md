...
- `src/vector_space.ac` adds field-specific lemmas: `vector_space_smul_ne_zero` (nonzero scalar times nonzero vector is nonzero), `vector_space_smul_inverse_left/right` (multiplying by inverse undoes scalar multiplication), and `vector_space_smul_left_cancel`/`vector_space_smul_right_cancel` (cancellation of nonzero scalar/vector).
