...
- `src/module_hom.ac` introduces a predicate-level linear-equivalence API: `is_linear_equiv` (linear map + bijection), with `linear_equiv_is_linear_map`/`linear_equiv_is_bijection` projections, `identity_fn_is_linear_equiv`, `compose_is_linear_equiv`, `linear_map_two_sided_inverse_preserves_add`/`..._preserves_smul`/`..._is_linear_map` (showing any two-sided inverse of a linear map is itself linear in the opposite direction), and `linear_equiv_two_sided_inverse_is_linear_equiv`.
