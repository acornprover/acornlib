...
- [ ] Add an indiscrete-topology instance: prove the `big_union` axiom on `s = empty or s = universal`. The helper `big_union_of_empty_family` is in place; an analogous "everywhere-empty-or-universal" lemma is still needed
