...
- [ ] Define the Bernoulli pmf on `Bool` with parameter `p` and compute its expectation and variance. Started in `bernoulli_pmf.ac`: `bernoulli_mass`, `bernoulli_support` (a two-element `FiniteSet[Bool]`), `bernoulli_support_contains_all`, `bernoulli_support_sum = 1`, `bernoulli_mass_nonneg`, `one_sub_nonneg`. Still to do: finish `bernoulli_is_pmf` (blocked on a vacuous `not support.contains(x) implies mass(x) = 0` step where Acorn rejects "inconsistent assumptions"), wrap it as a `DiscretePMF[Bool]`, and prove `E(indicator) = p` and `Var(indicator) = p * (1 - p)`.
