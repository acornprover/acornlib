...
- [ ] Kummer's theorem: `v_p(binom(m+n,m))` counts the base-`p` carries in `m+n`. Remaining work: unroll the recurrence into the floor-sum `Σ_i floor(n/p^i)`, define carries in base `p`, and prove `floor((a+b)/p^i) - floor(a/p^i) - floor(b/p^i)` is the carry indicator. The digit-sum form `(n - s_p(n))/(p-1)` falls out of the same unrolling.
