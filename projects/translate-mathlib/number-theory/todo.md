...
- [ ] Prove the digit-sum master identity `p * v_p(n!) + s_p(n) = n + v_p(n!)` in the regular library. The remaining issue is the induction over `digit_sum`: direct proof search still unfolds the `satisfy`-defined `div`/`mod` witnesses too eagerly. Next regular-library paths: add a `div`/`mod`-free digit-list representation and relate it to `digit_sum`, or prove small opaque recurrence lemmas around `digit_sum_fuel` that hide the witness definitions.
