...
- [ ] Prove the digit-sum master identity `p * v_p(n!) + s_p(n) = n + v_p(n!)` in the regular library. The fuel-recursion obstacle is reduced: `digit_sum_recurrence` now gives the clean step `s_p(n) = n mod p + s_p(n div p)` for `1 < p` and `n != 0`, hiding the `satisfy`-defined `div`/`mod` witnesses. Next path: combine this recurrence with `legendre_factorial_recurrence` and `div_mod_decomp` in an induction on `n`; the remaining likely work is explicit arithmetic normalization around `p * (n.div(p) + v)` and `n.div(p) * p + n.mod(p) = n`.
