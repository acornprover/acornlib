...
- [x] Legendre's formula in computable recurrence form: `legendre_recurrence` in `src/number_theory/legendre_recurrence.ac` proves `Σ_{k≤n} v_p(k) = (n div p) + Σ_{k≤(n div p)} v_p(k)` for prime `p`. With `legendre_factorial` and `legendre_binom` this makes `v_p(n!)` and `v_p(binom)` computable by repeated division.
- [x] Base-`p` digit machinery: `digit_sum(p, n)` (sum of base-`p` digits, via a fuel-bounded recursion) with `div_lt`, `digit_sum_fuel_step`, `digit_sum_fuel_at_zero`, and `digit_sum_zero` in `src/nat/digit_sum.ac`, exposed through the `nat` interface.
- [ ] **(blocked — prover performance)** Kummer's theorem and the digit-sum Legendre form. The math reduces (via `legendre_recurrence`) to the **digit-sum master identity** `p * v_p(n!) + s_p(n) = n + v_p(n!)` (equivalently `(p-1) v_p(n!) = n - s_p(n)`), from which Kummer `(p-1) v_p(binom(a+b,a)) = s_p(a) + s_p(b) - s_p(a+b)` = #carries follows by `legendre_binom`. The induction is routine but the prover times out even on trivial sub-goals: `digit_sum`'s recursion calls the `satisfy`-defined `div`/`mod`, so eager unfolding pulls in existential reasoning and explodes the search. Targets recorded in `hard_problems/kummer_digit_sum.ac`. Next: a `div`/`mod`-free (e.g. digit-list) `digit_sum` the prover won't unfold, or prover support for opaque recursive definitions.
...
- [ ] Egyptian / unit-fraction infrastructure: finite sums of distinct reciprocals `1/n`, denominator bounds, and existence / greedy results (only isolated instances such as triangular reciprocals exist today)
