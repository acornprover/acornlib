...
- [x] Verified eighteenth simplification micro-batch: `src/finite_set_filter.ac` with matching certificates/manifests; full `verify` and full `./scripts/acorn-check-strict.sh` passed with `100610/100610 OK`, `0 searches performed`.
- [x] Verified nineteenth simplification micro-batch: `src/functions_bijection_cancellation.ac` and `src/functions_compose_factor_reflection.ac` with matching certificates/manifests; full `verify` and full `./scripts/acorn-check-strict.sh` passed with `100544/100544 OK`, `0 searches performed`.
- [x] Verified twentieth simplification micro-batch: `src/list_fold_bridge.ac`, `src/list_prefix_suffix.ac`, and `src/list_reverse_append_helpers.ac` with matching certificates/manifests; full `verify` and full `./scripts/acorn-check-strict.sh` passed with `100472/100472 OK`, `0 searches performed`.
...
- [x] Verified twenty-sixth simplification micro-batch: `src/hom_kernel_injective.ac` and `src/linear_equiv_submodule.ac` with matching certificates/manifests; full `verify` and full `./scripts/acorn-check-strict.sh` passed with `100268/100268 OK`, `0 searches performed`.
...
- [ ] Deferred tested simplification: upstream `src/group_hom_subgroup.ac` timed out at `subgroup_image_contains[G, H](f, s, a * b)`.
